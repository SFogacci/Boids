...
sf::ConvexShape setShape(bd::Boid const& b) // passaggio by const ref
{
  if (b.isPredator()) {
    ...
    predatorShape.setPoint(0, sf::Vector2f(b.getPosition().x - 15.f, b.getPosition().y - 10.f));
    predatorShape.setPoint(1, sf::Vector2f(b.getPosition().x + 15.f, b.getPosition().y));
    predatorShape.setPoint(2, sf::Vector2f(b.getPosition().x - 15.f, b.getPosition().y + 10.f));
    predatorShape.setPoint(3, sf::Vector2f(b.getPosition().x - 5.f, b.getPosition().y));
...
void gameLoop(bd::Flock& flock)
{
  ...
  sf::RenderWindow window(sf::VideoMode(windowDimensions.width, windowDimensions.height), "Boids", sf::Style::Default, settings);
  ...
  while (window.isOpen()) {
    ...
    auto modified_predator = flock.predator_evolution();
    flock.evolution();
    flock.setPredator(modified_predator);

    window.clear();

    window.draw(setShape(flock.getPredator()));
